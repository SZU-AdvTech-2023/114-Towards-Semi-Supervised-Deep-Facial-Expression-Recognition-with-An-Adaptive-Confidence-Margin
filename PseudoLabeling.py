...
parser = argparse.ArgumentParser(description='PyTorch PseudoLabeling Training')
...
parser.add_argument('--T1', default=10, type=int, help='')
parser.add_argument('--T2', default=30, type=int, help='')
parser.add_argument('--alpha', default=3, type=int, help='')
...
console_log_path = os.path.join(args.out, 'console_log_PL.txt')
log_path = os.path.join(args.out, 'log_PL.txt')
...
def main():
    ...
    # log文件表头
    logger.set_names(['Train Loss', 'Test Loss', 'Test Acc'])
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        _, train_acc, outputs_new, targets_new = validate(labeled_trainloader, model, criterion, epoch, use_cuda,
                                                          mode='Train Stats')

        test_loss, test_acc, _, _ = validate(test_loader, model, criterion, epoch, use_cuda, mode='Test Stats')
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, criterion_ce, epoch, use_cuda):
    ...
    for batch_idx in range(args.train_iteration):
        ...
        Lu = criterion_ce(outputs_u, max_idx)
        Lu = Lu.mean()
        loss = Lx + Lu * slow_rampup(epoch)
...
def save_checkpoint(states, is_best, checkpoint=args.out, filename='checkpoint_PL.pth.tar'):
    '''Saves checkpoint to disk 将checkpoint保存到磁盘'''
    filepath = os.path.join(checkpoint, filename)
    torch.save(states, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best_PL.pth.tar'))
...
def slow_rampup(current, rampup_length=args.epochs):
    """Slow rampup from https://arxiv.org/abs/1610.02242"""
    if current < args.T1:
        return 0
    elif current < args.T2:
        return (args.alpha * (current - args.T1)) / (args.T2 - args.T1)
    else:
        return args.alpha
