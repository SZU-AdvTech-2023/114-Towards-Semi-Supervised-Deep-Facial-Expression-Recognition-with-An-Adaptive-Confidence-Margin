...
console_log_path = os.path.join(args.out, 'console_log_mixmatch.txt')
log_path = os.path.join(args.out, 'log_mixmatch.txt')
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion_ce, criterion_train,
    ...
    for batch_idx in range(args.train_iteration):
        ...
        loss = Lx + args.lambda_u * linear_rampup(epoch) * Lu  # eq. (5)
...
def save_checkpoint(states, is_best, checkpoint=args.out, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best_mixmatch.pth.tar'))
