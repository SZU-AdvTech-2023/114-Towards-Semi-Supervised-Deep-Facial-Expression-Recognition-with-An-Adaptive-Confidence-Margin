...
console_log_path = os.path.join(args.out, 'console_log_baseline.txt')
log_path = os.path.join(args.out, 'log_baseline.txt')
...
def main():
    ...
    train_set, test_set = dataset.get_baseline_raf(args.train_root, args.label_train,
                                                   args.test_root, args.label_test,
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        train_loss = train(train_loader, model, optimizer, criterion,  use_cuda)
        _, train_acc, outputs_new, targets_new = validate(train_loader, model, criterion, epoch, use_cuda,
                                                          mode='Train Stats')
...
def train(train_loader, model, optimizer, criterion_ce, use_cuda):
    ...
    for batch_idx in range(args.train_iteration):
        ...
        loss = criterion_ce(output_x, targets_x.long()).mean()
...
def save_checkpoint(states, is_best, checkpoint=args.out, filename='checkpoint_baseline.pth.tar'):
    '''Saves checkpoint to disk 将checkpoint保存到磁盘'''
    filepath = os.path.join(checkpoint, filename)
    torch.save(states, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best_baseline.pth.tar'))
