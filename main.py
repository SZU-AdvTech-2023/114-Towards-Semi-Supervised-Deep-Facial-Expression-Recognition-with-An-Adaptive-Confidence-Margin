...
parser = argparse.ArgumentParser(description='PyTorch Ada Training')
...
console_log_path = os.path.join(args.out, 'console_log.txt')
log_path = os.path.join(args.out, 'log_new_margin.txt')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # 获取特征向量 t-sne绘图
        outputs_np = outputs_new.cpu().numpy()
        ...
        scatter = plt.scatter(outputs_tsne[:, 0], outputs_tsne[:, 1], c=targets_new.cpu().numpy(), cmap='viridis')  # 使用目标标签作为颜色
        # 创建图例，将颜色映射到类别标签
        legend_labels = [f"{label}: {class_f}" for label, class_f in class_to_color.items()]
        plt.legend(handles=scatter.legend_elements()[0], labels=legend_labels, title='Labels', loc='upper right')
        plt.title('t-SNE Visualization of Model Outputs' + "-epoch-{}".format(epoch))
        plt.savefig('t-SNE Visualization of Model Outputs epoch-{}.png'.format(epoch))
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion_ce, criterion_simclr,
    ...
    for batch_idx in range(args.train_iteration):
        ...
        try:
            (inputs_u, inputs_u2, inputs_u3, inputs_strong), _ = next(unlabeled_train_iter)
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            (inputs_u, inputs_u2, inputs_u3, inputs_strong), _ = next(unlabeled_train_iter)
        ...
        # output_strong_2, _ = model(inputs_u3)
        # output_ave = (output_strong + output_strong_2) / 2
...
def adaptive_threshold_generate_new(outputs, targets, threshold, epoch):
    '''
    自适应阈值生成
    '''
    outputs_l = outputs[1:, :]
    ...
    for i in range(7):
        idx = np.where(targets_new.cpu() == i)[0]
        if idx.shape[0] != 0:
            threshold[i] = probs_new[idx].mean().cpu() * 0.97 / (1 + math.exp(-1 * (math.log(2,epoch+1)))) if probs_new[
                                                                                                  idx].mean().cpu() * 0.97 / (
                                                                                                          1 + math.exp(
                                                                                                      -1 * (math.log(2,epoch+1)))) >= 0.8 else 0.8
        else:
            threshold[i] = 0.8
    return threshold
