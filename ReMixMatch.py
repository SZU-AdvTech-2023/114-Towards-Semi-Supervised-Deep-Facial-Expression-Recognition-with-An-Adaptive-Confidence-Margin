...
# 设置训练数据
train_root, test_root, train_label_root, test_label_root, start_model_path = set_datasets("A")
...
console_log_path = os.path.join(args.out, 'console_log_remixmatch.txt')
log_path = os.path.join(args.out, 'log_remixmatch.txt')
...
def main():
    ...
    console_logger = ConsoleLogger(console_log_path, title='console_log', console_content='')
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        train_loss, train_loss_x, train_loss_u = train(labeled_trainloader, unlabeled_trainloader,
                                                                       ...
                                                                       criterion_train, threshold, epoch, use_cuda)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion_ce, criterion_train,
    ...
    for batch_idx in range(args.train_iteration):
        ...
        targets_x = torch.zeros(size=(batch_size, 7)).scatter_(1, targets_x.long().view(-1, 1), 1)

        # rotation strong augmented unlabeled data
        rot_y, rot_l = random_rotate(inputs_u)
        ...
        logits_rot, _ = model(rot_y)
        logits_ox, _ = model(inputs_x)
        ...
        p = p * (torch.softmax(logits_ox, dim=1)).mean()/p.mean()
        pt = p ** (1 / args.T)
        ...
        outputs_u_s, _ = model(inputs_strong)

        # mixup
        all_inputs = torch.cat([inputs_x, inputs_u, inputs_u2], dim=0)
        all_targets = torch.cat([targets_x, pseudo_label, pseudo_label], dim=0)
        ...
        # model forward
        mixuped_logits, _ = model(mixed_input)  # [3*N,10]
        logits_x = mixuped_logits[:args.batch_size]  # [N,10]
        logits_u = mixuped_logits[args.batch_size:]  # [2*N,10]

        # eq. (2) - (5)
        loss_rot = F.cross_entropy(logits_rot, rot_l)
        Lx, Lu = criterion_train(logits_x, mixed_target[:args.batch_size], logits_u,
                                         mixed_target[args.batch_size:])
        Lus = (F.cross_entropy(outputs_u_s, targets_u,reduction='none')).mean()
        ...
        losses_u_s.update(Lus.item(), inputs_x.size(0))
        losses_r.update(loss_rot.item(), inputs_x.size(0))
...
def save_checkpoint(states, is_best, checkpoint=args.out, filename='checkpoint.pth.tar'):
    '''Saves checkpoint to disk 将checkpoint保存到磁盘'''
    filepath = os.path.join(checkpoint, filename)
    torch.save(states, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best_remixmatch.pth.tar'))
